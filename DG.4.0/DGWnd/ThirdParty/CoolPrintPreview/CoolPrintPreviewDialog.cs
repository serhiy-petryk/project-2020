    ...
    public PrintDocument Document {
      ...
      set {
        ...
        if (_doc != null) {
          _doc.BeginPrint += _doc_BeginPrint;
          _doc.EndPrint += _doc_EndPrint;
//          cbPrintRange.Text = cbPrintRange.Items[_doc.PrinterSettings.PrintRange == PrintRange.Selection ? 1 : 0].ToString();
        }
    ...
    void _btnPrint_Click(object sender, EventArgs e) {
      using (PrintDialog dlg = new PrintDialog()) {
        ...
        bool flagSelection = (ps.PrintRange == PrintRange.Selection);
        dlg.AllowSelection = flagSelection;

        // show dialog
        if (dlg.ShowDialog(this) == DialogResult.OK) {
          // print selected page range
//          _preview.Print();
          _preview.Document.Print();
    ...
    void _btnPageSetup_Click(object sender, EventArgs e) {
      using (PageSetupDialog dlg = new PageSetupDialog()) {
        dlg.Document = Document;
        dlg.EnableMetric = System.Globalization.RegionInfo.CurrentRegion.IsMetric;
        if (dlg.ShowDialog(this) == DialogResult.OK) {
    ...
    void _doc_BeginPrint(object sender, PrintEventArgs e) {
      _btnCancel.Text = "Зупинити завантаження";
      _btnCancel.BackColor = Color.Yellow;
      _btnPrint.Enabled = _btnPageSetup.Enabled = false;
    }
    void _doc_EndPrint(object sender, PrintEventArgs e) {
      _btnCancel.Text = "Закрити";
      _btnCancel.BackColor = SystemColors.Control;
...
/*    private void btnPrintDialog_Click(object sender, EventArgs e) {
    ...
    private void cbPrintRange_TextChanged(object sender, EventArgs e) {
      bool printAll = this.cbPrintRange.Items[0].ToString() == this.cbPrintRange.Text;
      bool currentPrintAll = this._doc.PrinterSettings.PrintRange!= PrintRange.Selection;
      if (printAll != currentPrintAll) {
        this._doc.PrinterSettings.PrintRange = this.cbPrintRange.Items[0].ToString() == this.cbPrintRange.Text ? PrintRange.AllPages : PrintRange.Selection;
        //Deactivate current combobox (RefreshPreview is doing a long time)
        this.cbPrintRange.Owner.TopLevelControl.Select();
        Application.DoEvents();
        _preview.RefreshPreview();
      }
    }*/
