//http://www.danielsoper.com/programming/
...
namespace DGWnd.Useful {
  ...
  /// ******************************************************************
  /// 
  public class DGV_ReoderColumnsAndRows : DataGridView {
    ...
    private int DragDropType=-1; //0=column, 1=row
    private DataGridViewColumn DragDropColumn;
    private object[] DragDropColumnCellValue;

    private bool _allowCustomColumnReoder=true;
    private bool _allowCustomRowReoder=true;

    public DGV_ReoderColumnsAndRows() {
...
//      this.Size = new Size(600, 400);
    ...
    [DefaultValue(true)]
    public bool _AllowCustomColumnReoder {
      get { return this._allowCustomColumnReoder; }
      set { this._allowCustomColumnReoder = value; }
    }
    [DefaultValue(true)]
    public bool _AllowCustomRowReoder {
      get { return this._allowCustomRowReoder; }
      set { this._allowCustomRowReoder = value; }
    }
      ...
      if (e.RowIndex > -1) {
        if (e.Button == MouseButtons.Left) {
          ...
          if (this.SelectionMode != DataGridViewSelectionMode.RowHeaderSelect) {
            ...
            if (e.ColumnIndex >= 0)
              this.CurrentCell = this[e.ColumnIndex, e.RowIndex];// ??? e.columnindex<0
          } //end if
        }
        else if (e.Button == MouseButtons.Right) {
          //single-click right mouse button
...
//          if (this.Rows[this.HitTest(e.X, e.Y).RowIndex].Selected) {
          if (this.Rows[this.HitTest(e.X, e.Y).RowIndex].Selected && this._allowCustomRowReoder) {
...
//        else if (this.HitTest(e.X, e.Y).ColumnIndex > -1 && this.HitTest(e.X, e.Y).RowIndex == -1) {
        else if (this.HitTest(e.X, e.Y).ColumnIndex > -1 && this.HitTest(e.X, e.Y).RowIndex == -1 && this._allowCustomColumnReoder) {
        ...
        if (drgevent.Effect == DragDropEffects.Move) {
          ...
          if (DragDropType == 0) {
            ...
            if (DragDropTargetIndex > -1 && DragDropCurrentIndex < this.RowCount - 1) {
              ...
              // IList support added by Sergei Petrik
              object ds = ListBindingHelper.GetList(this.DataSource, this.DataMember);
              if (ds is IList) {
                IList bl = (IList)ds;
                object item=SourceRow.DataBoundItem;
                bl.RemoveAt(DragDropSourceIndex);
                bl.Insert(DragDropTargetIndex, item);
                this.Invalidate();// Remove red line and refresh cell values
              }
              else {
        ...
        if (DragDropType == 0) {
          ...
          if (e.RowIndex == DragDropCurrentIndex && DragDropCurrentIndex < this.RowCount - 1) {
            ...
            Pen p = new Pen(Color.Red, 2);
              e.Graphics.DrawLine(p, e.CellBounds.Left, e.CellBounds.Top-2 , e.CellBounds.Right, e.CellBounds.Top-2 );
          } //end if
        } //end if
      } //end if
