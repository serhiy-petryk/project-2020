    ...
    public static class ProfileYahoo_Parse
    {
        public static void Start(string folder, Action<string> showStatusAction)
        {
            ...
            foreach (var file in files)
            {
                cnt++;
                if (cnt % 10 == 0)
                    showStatusAction($"ProfileYahoo_Parse processed {cnt:N0} from {files.Length:N0} files");

                var fileSymbol = Path.GetFileNameWithoutExtension(file).Split('_')[1];
                var timeStamp = DateTime.ParseExact(Path.GetFileNameWithoutExtension(file).Split('_')[2], "yyyyMMdd",
                ...
                if (i1 > 0)
                {
                    ...
                    {
                        Symbol = symbol, Exchange = exchange, Name = name, Type = "stock", Sector = sector,
                        Industry = industry, Employees = employees, TimeStamp = timeStamp
                    };
                    items.Add(item);
                }
                else
                {
                    ...
                    if (i1 > 0)
                    {
                        ...
                        var item = new Models.ProfileYahoo
                        {
                            Symbol = symbol, Exchange = exchange, Name = name, Type = "etf", Category = category,
                            Family = fondFamily, NetAssets = netAssets, YtdDailyTotalReturn = ytdDailyTotalReturn,
                            Yield = yield, LegalType = legalType, TimeStamp = timeStamp
                        };
                        items.Add(item);
                    }
                    else
            ...
            var errorLogFileName = Directory.GetParent(folder) + @"\errorLog.txt";
